...
var updateCommitActivity = function (githubOrgName, mozTeamName, githubRepoName, allMembersList, since, callback) {
  ...
  if (since) msg.since = since.toISOString(); // if no 'since' is specifified this will fetch full repo history
  ...
  // New version in progres
  // // Create an async queue to process saving while other requests are made of API
  // var q = async.queue(function queued (task, callback) {
  //   // TODO add in here the functionality to save
  //   console.log("Added to Queue: ", task.githubOrgName);
  ...
  // async.waterfall([
  //     function fetchedAllThePages(callback){
  //       // iterate through all pages of the API, adding all responses into the queue to save
  //       // TODO - add in iteration functionality here
  //       // TODO - these seem to all be pushed to the queue in one go - Need to test some delays and make sure the queue gets processed in the meantime
  //       q.push({"githubOrgName": "One", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Two", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Three", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Four", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Five", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Six", "mozTeamName": "Webmaker"});
  //       q.push({"githubOrgName": "Seven", "mozTeamName": "Webmaker"});
  //       callback(null);
  //     },
  //     function checkedTheQueueIsEmpty (callback){
  //       // when all the iteration through the API is complete, wait here until the saving is complete
  //       // i.e. when the queue has been processed
  //       async.whilst(function truth_test () {
  //           return (q.length() > 0);
  //         }, function testing () {
  ...
  // existing working code to make more effecient
