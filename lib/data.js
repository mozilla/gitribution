...
exports.resetDatabaseYesIreallyWantToDoThis = function resetDatabaseYesIreallyWantToDoThis(callback) {
  pool.getConnection(function connectionAttempted (err, connection) {
    if (err) console.log(err);
      ...
      connection.query('TRUNCATE members', function queryComplete (err, result) {
        if(err) console.log(err);
        else {
...
* CONTRIBUTION ACTION
*/
exports.saveItem = function saveItem(happenedOn, githubOrgName, githubRepo, githubLogin, githubPublicEmail, actionType, mozTeamName, callback) {
...
* MEMBER
*/
exports.saveMember = function saveMember(githubOrgName, githubLogin, callback) {
  pool.getConnection(function(err, connection) {
    if (err) callback(err);
    else {
      var member = {
      ...
      // There is a unique key set across all the fields
      connection.query('REPLACE INTO members SET ?', member, function(err, result) {
        if(err) console.log(err);
        connection.release();
        callback(null);
      });
